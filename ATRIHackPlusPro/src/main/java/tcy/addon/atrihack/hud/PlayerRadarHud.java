...
public class PlayerRadarHud extends HudElement {
    public static final HudElementInfo<PlayerRadarHud> INFO = new HudElementInfo<>(ATRIHack.HUD_BLACKOUT, "player-radar+", "Displays players in your visual range.", PlayerRadarHud::new);
    ...
    private final Setting<SettingColor> secondaryColor2 = sgGeneral.add(new ColorSetting.Builder()
        .name("secondary-color-2")
        .description("Secondary color.")
        .defaultValue(new SettingColor(255, 175, 175))
        .build()
    );
    ...
    public void tick(HudRenderer renderer) {
        ...
        for (PlayerEntity entity : getPlayers()) {
            ...
            if (distance.get() && health.get()) {
                text += String.format("(%sm)", Math.round(mc.getCameraEntity().distanceTo(entity))) + String.format("(%s\u2665)",Math.round(entity.getHealth()));
            }else if (health.get()){
                text += String.format("(%s\u2665)",Math.round(entity.getHealth()));
    ...
    public void render(HudRenderer renderer) {
        ...
        renderer.text("->", x + border.get() + alignX(renderer.textWidth("->", shadow.get(), getScale()), alignment.get()), y, secondaryColor.get(), shadow.get(), getScale());
        ...
        for (PlayerEntity entity : getPlayers()) {
            ...
            if (health.get()){
                healthText = String.format("(%s\u2665)",Math.round(entity.getHealth()));
            ...
            if (health.get()) renderer.text(healthText, x + spaceWidth, y,secondaryColor.get(), shadow.get(), getScale());
        }
    }
