...
import tcy.addon.atrihack.enums.HoleType;
...
public class PistonPush extends ArkModule {
    public PistonPush() {
        super(ATRIHack.atricombot, "Hole Push", "Pushes people out of their safe holes.");
    }
        ...
        switch (renderMode.get()) {
            case Smooth -> {
                ...
                if (r >= 0.001) {
                    ...
                    double up = -0.5;
                    double width = 0.5;
                    Box pisbox = new Box(pistonPos.getX() + 0.5 - width, pistonPos.getY() + down, pistonPos.getZ() + 0.5 - width,
                        pistonPos.getX() + 0.5 + width, pistonPos.getY() + up, pistonPos.getZ() + 0.5 + width);

                    Box redbox = new Box(redstonePos.getX() + 0.5 - width, redstonePos.getY() + down, redstonePos.getZ() + 0.5 - width,
                        redstonePos.getX() + 0.5 + width, redstonePos.getY() + up, redstonePos.getZ() + 0.5 + width);

                    event.renderer.box(pisbox, psColor.get(), plColor.get(), pistonShape.get(), 0);
                    event.renderer.box(redbox, rsColor.get(), rlColor.get(), redstoneShape.get(), 0);
                }
            }
            case Easy -> {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        Vec3d rPos = new Vec3d(pistonPos.getX() + 0.5, pistonPos.getY() + 0.7, pistonPos.getZ() + 0.5);
