...
import static tcy.addon.atrihack.utils.timers.TimerUtils.getTPSMatch;

public class StrafePlus extends ArkModule {
    public enum Mode {
        Vanilla,
        NCP,
        Smart
    ...
    public StrafePlus() {
            super(ATRIHack.ArkMode, "strafe+", "Increase speed and control.");
    }
    ...
    AnchorAuraPlus anchor = Modules.get().get(AnchorAuraPlus.class);
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (EntityUtils.isWebbed(mc.player) && webbedPause.get() == WebbedPause.Always) return;
        if (EntityUtils.isWebbed(mc.player) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
    ...
    private void onPreTick(TickEvent.Pre event) {
        ...
        if (EntityUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && webbedPause.get() == WebbedPause.Always) return;
        if (EntityUtils.doesBoxTouchBlock(mc.player.getBoundingBox(), Blocks.COBWEB) && !mc.player.isOnGround() && webbedPause.get() == WebbedPause.OnAir) return;
