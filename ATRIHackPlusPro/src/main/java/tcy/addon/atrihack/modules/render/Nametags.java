...
public class Nametags extends Module {
    ...
    private final double[] itemWidths = new double[12];

    private final List<Entity> entityList = new ArrayList<>();

    public Nametags() {
        super(ATRIHack.atrirender, "NameTagsTest", "Displays customizable nametags above players.");
    }
    ...
    private void renderNametagPlayer(Render2DEvent event, PlayerEntity player, boolean shadow) {
        ...
        double health = Math.round((player.getHealth() + absorption) * 10000.00000) / 10000.00000;
        double healthPercentage = health / (player.getMaxHealth() + absorption);
        String healthText = " " + health;
        Color healthColor;

        if (healthPercentage <= 0.3660004) healthColor = RED;
        else if (healthPercentage <= 0.6660004) healthColor = AMBER;
    ...
    private void renderGenericNametag(LivingEntity entity, boolean shadow) {
        ...
        String formattedHealthPercentage = decimalFormat.format(healthPercentage);
        //new---------------------------------
