...
import tcy.addon.atrihack.utils.PlaceData;
import tcy.addon.atrihack.utils.RotationUtils;
import tcy.addon.atrihack.utils.SettingUtils;
import tcy.addon.atrihack.utils.player.DamageInfo;
...
public class BedBombV4 extends ArkModule {
    public BedBombV4() {
        super(ATRIHack.atricombot, "BedAura new", "Automatically places and breaks beds to cause damage to your opponents in strict anticheat servers.");
    }
...
//    }
    private void update() {
        ...
        if (logicMode.get() == LogicMode.PlaceBreak) {
            List<BlockPos> interactBlocks = interactUpdate();
            if (interactBlocks != null && !interactBlocks.isEmpty()) {
                interactBlocks.forEach(this::removeBed);
            }
            ...
            if (LemonUtils.replaceable(placePos) && LemonUtils.replaceable(placePos.offset(bedDir)) && placeUpdate()) {
                updateBeds(placePos, placePos.offset(bedDir));
                timer = 0;
            }
        } else {
            if (!isBed(placePos) && !isBed(placePos.offset(bedDir)) && placeUpdate()) {
    ...
    private void updateBeds(BlockPos pos1, BlockPos pos2) {
        removeBed2(pos1);
        beds.add(new Bed(pos1, pos2, true, System.currentTimeMillis()));
    }
