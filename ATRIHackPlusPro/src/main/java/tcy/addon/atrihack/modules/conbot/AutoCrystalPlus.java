...
import tcy.addon.atrihack.mixins.IInteractEntityC2SPacket;
import tcy.addon.atrihack.modules.player.AutoMine;
import tcy.addon.atrihack.modules.player.Suicide;
import tcy.addon.atrihack.timers.TimerList;
...
import tcy.addon.atrihack.utils.meteor.BOEntityUtils;
...
public class AutoCrystalPlus extends ArkModule {
    public AutoCrystalPlus() {
        super(ATRIHack.atricombot, "Auto Crystal+", "Breaks and places crystals automatically (but better).");
    }
    ...
    private final Setting<Double> renderTime = sgRender.add(new DoubleSetting.Builder()
        ...
        .min(0)
        .sliderRange(0, 10)
        .visible(() -> renderMode.get().equals(RenderMode.Earthhack) || renderMode.get().equals(RenderMode.Future)|| renderMode.get().equals(RenderMode.ATRI))
        .build()
    );
    private final Setting<Double> renderhigh = sgRender.add(new DoubleSetting.Builder()
        .name("Render High")
        .description("")
        .defaultValue(0.2)
        .min(0)
                ...
                case ATRI -> {
                    ...
                    if (renderProgress > 0 && renderPos != null) {
                        event.renderer.box(new Box(renderPos.getX(), renderPos.getY() , renderPos.getZ(),
                                renderPos.getX() + 1, renderPos.getY()+0.2, renderPos.getZ() + 1),
                            new Color(color.get().r, color.get().g, color.get().b, (int) Math.round(color.get().a * Math.min(1, renderProgress / fadeTime.get()))),
                            new Color(lineColor.get().r, lineColor.get().g, lineColor.get().b, (int) Math.round(lineColor.get().a * Math.min(1, renderProgress / fadeTime.get()))), shapeMode.get(), 0);
                    }
                }
                case ATRI1 -> {
                    ...
                    if (renderPos != null) {
                        ...
                        if (r >= 0.001) {
                            ...
                            int a=0;
                                Box box = new Box(renderPos.getX() + 0.5 - width, renderPos.getY() + down + 1, renderPos.getZ() + 0.5 - width,
                                    renderPos.getX() + 0.5 + width, renderPos.getY() + up + renderhigh.getDefaultValue(), renderPos.getZ() + 0.5 + width);
                                event.renderer.box(box, new Color(color.get().r, color.get().g, color.get().b, Math.round(color.get().a)), lineColor.get(), shapeMode.get(), 0);
                        }
                    }
                }
    ...
    private boolean attackDelayCheck() {
        if (instantAttack.get()) {
            return expSpeedLimit.get() <= 0 || System.currentTimeMillis() > lastAttack + 900 / expSpeedLimit.get();
        } else {
            return System.currentTimeMillis() > lastAttack + 900 / expSpeed.get();
        }
    }
    ...
    private void placeCrystal(BlockPos pos, Direction dir, Hand handToUse, int sl, int hsl) {
        if (pos != null && mc.player != null) {
            ...
            blocked.add(new Box(pos.getX() - 0.3, pos.getY() + 1, pos.getZ() - 0.3, pos.getX() + 1.3, pos.getY() + 2, pos.getZ() + 1.3));
            ...
            if (idPredict.get()) {
                ...
                for (int i = 0; i < idPackets.get() * idOffset.get(); i += idOffset.get()) {
                    addPredict(id + i, new Vec3d(pos.getX() + 0.3, pos.getY() + 1, pos.getZ() + 0.3), idDelay.get() + idPacketDelay.get() * i);
                }
            }
        }
    }
