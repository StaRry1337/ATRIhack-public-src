...
public class HoleESPPlus extends ArkModule {
    ...
    public HoleESPPlus() {
            super(ATRIHack.atrirender, "hole-esp+", "Displays holes that you will take less damage in.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockIterator.register(horizontalRadius.get(), verticalRadius.get(), (blockPos, blockState) -> {
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (EntityUtils.isBlastResistant(blockPos.offset(direction), EntityUtils.BlastResistantType.Unbreakable)) bedrock++;
                else if (EntityUtils.isBlastResistant(blockPos.offset(direction), EntityUtils.BlastResistantType.Mineable)) blastproof++;
                else if (direction == Direction.DOWN) return;
                else if (validHole(blockPos.offset(direction)) && air == null) {
                    for (Direction dir : Direction.values()) {
                        if (dir == direction.getOpposite() || dir == Direction.UP) continue;

                        if (EntityUtils.isBlastResistant(blockPos.offset(direction).offset(dir), EntityUtils.BlastResistantType.Unbreakable)) bedrock++;
                        else if (EntityUtils.isBlastResistant(blockPos.offset(direction).offset(dir), EntityUtils.BlastResistantType.Mineable)) blastproof++;
                        else return;
                    }
