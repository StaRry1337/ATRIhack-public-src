...
package tcy.addon.atrihack.utils.meteor;
...
public class BODamageUtils {
    ...
    public static double bedDamage(LivingEntity player, Box box, Vec3d bed, BlockPos ignore,Vec3d pos) {
        if (player instanceof PlayerEntity && ((PlayerEntity) player).getAbilities().creativeMode) return 0;

        double modDistance = Math.sqrt(pos.squaredDistanceTo(bed));
    ...
    // Anchor damage
    public static double anchorDamage(LivingEntity player, Box box, BlockPos anchor,Vec3d pos) {
        return bedDamage(player, box, anchor.toCenterPos(), anchor,pos);
    }
